...
import { CheckCircle, XCircle, PauseCircle, KeyRound, Users, Clock, UserCheck, Ban } from 'lucide-react';
        ...
        <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
          {(['all','pending','approved','rejected','suspended'] as Filter[]).map(f=>(
            <button key={f} onClick={()=>setFilter(f)}
              className={`px-3 py-1.5 rounded-md text-xs font-semibold capitalize transition-colors ${filter===f?'bg-white dark:bg-gray-700 shadow-sm text-gray-900 dark:text-white':'text-gray-500 hover:text-gray-800 dark:hover:text-gray-200'}`}>
              {f} {fc[f]>0&&<span className="ml-1 text-[10px] bg-brand-100 dark:bg-brand-900/30 text-brand-700 dark:text-brand-400 px-1.5 rounded-full">{fc[f]}</span>}
            </button>
          ))}
        </div>
        <input className="input w-52 text-sm" placeholder="Search company, username, email…" value={search} onChange={e=>setSearch(e.target.value)}/>
      </div>
